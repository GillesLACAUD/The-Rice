...
#define GRAIN_MAX				6		// Max number of grain
...
GRAIN_EXTRN uint32_t	u32_Grain_PosStart_BigBuffer;				// Position of the big buffer in the file
GRAIN_EXTRN uint32_t	u32_Grain_PosOffset_BigBuffer;				// Compteur when the buffer is play
GRAIN_EXTRN uint8_t		u8_Grain_TrigBigBuffer_Full;				// Check if the buffer is full
GRAIN_EXTRN uint32_t	u32_Grain_AfterLFO;
GRAIN_EXTRN uint32_t	u32_Grain_Slice;							// Size of the Grain in Slice


#define BUFFER_MONO		1
#define BUFFER_STEREO	2

GRAIN_EXTRN uint8_t		u8_Type_BigBuffer;
GRAIN_EXTRN uint8_t		u8_Ask_Redraw;
GRAIN_EXTRN uint8_t		u8_Ask_DrawGrain;
...
#define ADSR_DIV_POT				ADSR_MAX_CPT/500
...
typedef struct
{
	...
	uint8_t 	u8_buffcumul[AUDIO_OUT_BUFFER_SIZE/2];
	uint16_t	u16_cpt_buffersrc;
	uint16_t	u16_cpt_bufferdst;
	int16_t		i16_srcsample;
...
#define AG_GRAIN_SUSTAIN_ON		1
#define AG_GRAIN_SUSTAIN_OFF	0
...
typedef struct
{
	...
	uint16_t	u16_Grain_NextTrig;				// Trig the Next grain after x% of the size play
	uint32_t	u32_offsetnextgrain;			// Offset between the Grain
	uint8_t		u8_rangeshift;					//
	uint8_t		u8_spaceranfixe;				// Space between the grains Fixe or Random
	...
	uint8_t		u8_sustain;						// AMP Sustain ON or OFF
	uint32_t	u32_deltaA;						//
	uint32_t	u32_deltaR;						//
	...
	// General Volume
	uint32_t	u32_Grain_GeneralVolume;		//
	...
	uint8_t		u8_Grain_NextPourcent;
	uint8_t		u8_adsr1bypass;
	uint8_t		u8_adsr2bypass;

	uint8_t	MIDIDroneNote[4];
	float	MIDIDroneDetune[4];

	uint8_t		Tab_Extend[200];				// For future extension
...
GRAIN_EXTRN str_Sound	str_Sound_Work;


GRAIN_EXTRN uint8_t		u8_AskChangeGrain;		//
...
GRAIN_EXTRN void	Grain_Set_AR();
GRAIN_EXTRN void	Grain_Set_LFO(stLfo* ptLFO);
GRAIN_EXTRN void	Grain_Set_EG();
GRAIN_EXTRN uint8_t Grain_Init(uint8_t actif,uint8_t nbgr,uint32_t posinit,uint16_t	sizeinit,uint16_t inter);
